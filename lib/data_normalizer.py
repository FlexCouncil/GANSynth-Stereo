...
class SpecgramsPrespecifiedNormalizer(object):
  ...
  def __init__(self, config):
    ...
    # stereo
    self._a = np.asarray([m_a, p_a, m_a, p_a])[None, None, None, :]
    self._b = np.asarray([m_b, p_b, m_b, p_b])[None, None, None, :]
    # mono
    self._c = np.asarray([m_a, p_a])[None, None, None, :]
    self._d = np.asarray([m_b, p_b])[None, None, None, :]
  ...
  def normalize_op(self, x):
    ...
    # Scan for stereo or mono
    num_channels = x.shape[3]
    if num_channels == 4:
      return tf.clip_by_value(self._a * x + self._b, -1.0, 1.0)
    else:
      return tf.clip_by_value(self._c * x + self._d, -1.0, 1.0)
