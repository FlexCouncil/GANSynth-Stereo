...
def generator(z,
  ...
  Args:
    ...
    colors: Number of output color channels. Defaults to 4 (for stereo).
  ...
  def _to_rgb(x):
    # print('NETWORKS--INTO _TO_RGB--PADDING')
    # print(padding)
    return layers.custom_conv2dR(
        x=x,
        filters=colors,
        kernel_size=1,
        padding='SAME',
  ...
  with tf.variable_scope(scope, reuse=reuse):
    ...
    with tf.variable_scope(block_name(1)):
      if simple_arch:
        ...
        # x = layers.attention(x, num_filters_fn(1))
      else:
    ...
    for block_id in range(2, num_blocks + 1):
      with tf.variable_scope(block_name(block_id)):
        if simple_arch:
          ...
          if (x.shape[1] * x.shape[2] == 8192):
            x = layers.attention_2(x, num_filters_fn(block_id))
        lods.append(x)
    ...
    for block_id in range(1, num_blocks + 1):
      with tf.variable_scope(block_name(block_id)):
        if simple_arch:
          ...
          # lod = layers.attention(lod, num_filters_fn(colors))
        else:
...
def discriminator(x,
  ...
  def _conv2dR(scope, x, kernel_size, filters, padding='SAME'):
    return layers.custom_conv2dR(
        x=x,
        filters=filters,
        kernel_size=kernel_size,
  ...
  def _from_rgb(x, block_id):
    return _conv2dR('from_rgb', x, 1, num_filters_fn(block_id))
  ...
  with tf.variable_scope(scope, reuse=reuse):
    ...
    for block_id in range(num_blocks, 1, -1):
      with tf.variable_scope(block_name(block_id)):
        if simple_arch:
          ...
          if (x.shape[1] * x.shape[2] == 8192):
            x = layers.attention_2(x, num_filters_fn(block_id - 1))
    ...
    with tf.variable_scope(block_name(1)):
      ...
      if simple_arch:
        ...
        x = tf.nn.relu(x)
        # x = layers.attention(x, num_filters_fn(0))
      else:
