...
import lib.network_functions as net_fns
...
def set_flags(flags):
  ...
  ### Dataset ###
  flags.set_if_empty('dataset_name', 'dataset_directory')
  flags.set_if_empty('channel_mode', 'stereo') # stereo, mono
  flags.set_if_empty('interpolate', 'no') # interpolate generated clip
  flags.set_if_empty('data_type', 'mel')  # linear, phase, mel
  flags.set_if_empty('audio_length', 1.0)
  flags.set_if_empty('sample_rate', 44100)
  ...
  flags.set_if_empty('generator_ac_loss_weight', 10.0)
  flags.set_if_empty('discriminator_ac_loss_weight', 10.0)
  ...
  flags.set_if_empty('generator_learning_rate', 0.0008)  # Learning rate
  flags.set_if_empty('discriminator_learning_rate', 0.0008)  # Learning rate
  ...
  flags.set_if_empty('fake_batch_size', 4)  # The fake image batch size
...
class Model(object):
  ...
  def __init__(self, stage_id, batch_size, config):
    ...
    """
    self._config = config
    self._audio_length = int(config['audio_length'] * config['sample_rate'])
  ...
  def add_summaries(self, stage_id):
    """Adds model summaries."""
    config = self.config
    data_helper = self.data_helper

    def _add_waves_summary(name, waves, max_outputs):
      waves_max = tf.reduce_max(waves)
      waves = waves / waves_max
      
      tf.summary.audio(name,
                       waves,
                       sample_rate=config['sample_rate'],
                       max_outputs=max_outputs)    
      fname = os.path.join('output_dir/', 'summary_stage_{}.wav'.format(stage_id))
  ...
  def generate_z(self, n):
    z_instruments = np.random.normal(size=[n, self.config['latent_vector_size']])
    return z_instruments

  def generate_samples(self, n_samples, pitch=None):
    ...
    return self.generate_samples_from_z(z, pitches)

  def generate_samples_from_z(self, z_notes, pitches):
    """Generate fake samples for given latents and pitches.

    Args:
      z_notes: A numpy array of latent vectors [n_samples, n_latent dims].
      pitches: An iterable list of integer MIDI pitches [n_samples].

    Returns:
      audio: Generated audio waveforms [n_samples, audio_length]
    """
    labels = self._pitches_to_labels(pitches)
    n_samples = len(labels)
    
    # if (self._config['interpolate'] == "no"):
    # Make all notes come from the same z instrument
    for i in range(n_samples):
    	idx = i - 1
    	z_notes[idx] = z_notes[0]
    ...
    z = np.concatenate([z_notes, np.zeros([padding, z_notes.shape[1]])], axis=0)
    ...
    for i in range(num_batches):
      ...
      # Trim waves
      for wave in waves:
        waves_list.append(wave[:self._audio_length, :])
  ...
  def get_audio_length(self):
    return self._config['audio_length']